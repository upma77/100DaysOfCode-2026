...
void insert(int id) {
    ...
    if (head == NULL) {
        head = newNode;                // First node should become head
        return;
    }
...
void display() {
    ...
    // Traverse until null, not until head
    while (temp != NULL) {
...
void deleteHero(int id) {
    ...
    free(temp);                        // Free deleted node, not previous node
}
...
struct Node* findMiddle() {
    ...
    // FIX 5: Fast pointer should move two steps
