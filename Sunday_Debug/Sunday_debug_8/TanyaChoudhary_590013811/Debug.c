...
void insert(int value) {
    ...
    int cur = heapSize++;
    while (cur > 0) {
        int par = (cur - 1) / 2;          // correct parent formula for 0-indexed heap
        if (heap[cur] > heap[par]) {      // '>' so larger values float up (max-heap)
...
int extractMax() {
    ...
    if (heapSize > 0) heapifyDown(0);     // no uninitialized variable
    return maxVal;
}
